 ...
 *    ("Or pay monthly — $49/mo") that keeps the monthly path discoverable.
 ...
 *  effective per-month price ($40.83 = $490/yr ÷ 12) with the "2 months free"
 *  framing from the pricing playbook. Centralised so every surface stays
 *  on-message.
 *
 *  Bug history (FIX-G, 2026-05-14): this constant previously read "$7.50/mo
 *  billed yearly" which is the Hobby Annual effective per-month price — a
 *  copy-paste from the Hobby card. Six upgrade surfaces (vault_prod,
 *  provision_twin, family_bindings, quota_wall, custom_domain, private_deploy)
 *  inherited the bug. Numbers are sourced from api/plans.yaml pro_yearly:
 *  49000 cents / 12 = $40.83/mo. If pricing changes again, update plans.yaml
 *  first, then this string + the priceLine entries below. */
 ...
 *  button.
 *
 *  Bug history (FIX-G, 2026-05-14): previously "$9/mo" (Hobby Monthly price).
 *  Pro Monthly is $49/mo per api/plans.yaml pro.price_monthly_cents=4900. */
