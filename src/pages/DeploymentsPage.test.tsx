...
describe('DeploymentsPage — empty state', () => {
  ...
  it('renders a 429 rate-limit hint with the Retry-After seconds', async () => {
    ...
    // The "No deployments yet" copy must NOT appear in the same render.
    // (data-testid="deployments-empty" still renders for the table's
    // honest empty-list cell, so we check the banner specifically.)
  })
})
...
describe('DeploymentsPage — non-empty state', () => {
  it('renders one row per deployment with name, URL, and status pill', async () => {
    ...
    // the agent API resolves `:id` against the app_id column. Routing by
    // UUID would 404. Linking to /app/deployments/* directly (rather
