...
export default defineConfig({
  ...
  test: {
    ...
    coverage: {
      // src/api/generated.ts is GENERATED by `npm run gen:api-types`
      // (openapi-typescript) — it is pure type declarations with no runtime
      // code, and is regenerated/up-to-date-gated separately
      // (gen:api-types:check). Exclude it from coverage so the org 100%-patch
      // gate (diff-cover, see .github/workflows/coverage.yml) never demands
      // tests for machine-generated types. Same rationale as excluding any
      // codegen artifact. The v8 provider's default excludes are preserved by
      // spreading coverageConfigDefaults.exclude.
      exclude: [...coverageConfigDefaults.exclude, 'src/api/generated.ts'],
    },
  },
});
